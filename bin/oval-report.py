...
def main(report: str):
  ...
  if not is_valid:
    raise ValueError('Cannot find target css under <style>, it is very likely \
  ...
  def identify(elem: ET.Element):
    ...
    tag = real_tag(elem)
    return '.'.join(
      [tag] + ([] if class_list is None else class_list.split(' ')))

  has_vuln = False
  def peek_vuln(elem: ET.Element, paths: list[str]):
    ident = identify(elem)
    if paths[-2:] != ['table', 'tr.LightRow.Center']:
      return False, ident
    if ident == 'td.SmallText.resultbadB':
      nonlocal has_vuln
      has_vuln = int(elem.text) > 0
      if has_vuln:
        print(f'[ERROR] {elem.attrib["title"]}: {elem.text}', file=sys.stderr)
    return has_vuln, ident

  is_valid = dfs(body, ['html'], peek_vuln)
  if not is_valid:
    raise ValueError('Cannot find target HTML element, it is very likely \
      that this script is outdated!')
  if not has_vuln:
    sys.exit(0)
